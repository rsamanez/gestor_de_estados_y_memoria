...
const isRedisEnvironment = process.env.REDIS_URL && process.env.REDIS_URL !== 'redis://default:your_password@your_redis_url:your_port';

console.log('🔧 Redis Environment Check:', {
  hasRedisUrl: !!process.env.REDIS_URL,
  isPlaceholder: process.env.REDIS_URL === 'redis://default:your_password@your_redis_url:your_port',
  willUseRedis: isRedisEnvironment
});
...
export async function getStore(key) {
  ...
  if (redis) {
    try {
      ...
      console.log(`📄 Redis GET result for ${key}:`, result ? 'FOUND' : 'NOT FOUND', result ? `(${typeof result})` : '');
...
export async function setStore(key, value) {
  ...
  if (redis) {
    try {
      ...
      const result = await redis.set(redisKey, serializedValue, { ex: ttlSeconds });
